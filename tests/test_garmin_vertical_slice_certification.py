...
from packages.connectors.garmin import GarminActivityPage, GarminCompletedActivityAdapter
...
class EvidenceStore:
    ...
    def persist_payload(
        ...
        *,
        ...
        reference = (
            f"evidence://garmin/{connection_id}/{source_record_id}/{payload_hash}"
        )
...
def pipeline(*, stop_at_schema: bool = False) -> ValidationPipeline:
    processors = [
        ...
        Processor(
            ...
            derived_payload={"distance_meters_candidate": 10000.0},
        ),
...
def test_integrated_garmin_path_reaches_shared_validation_and_authority_boundary() -> None:
    ...
    adapter = GarminCompletedActivityAdapter(
        client=Client(
            {None: GarminActivityPage((activity(),), next_cursor="c1", complete=True)}
        ),
    ...
    result = sync.run(
        adapter=adapter,
        connection_id="athlete-connection",
    ...
    validation = pipeline().run(
        PipelineContext(
            ...
            source_metadata={
                ...
                "source_record_id": envelope.record.source_record_id,
            },
        )
    )
    ...
    candidate = AuthorityCandidate(
        observation_id, "distance", "garmin", "observation://distance"
    )
...
def test_external_authority_controls_selection_without_garmin_precedence() -> None:
    ...
    candidate = AuthorityCandidate(
        selected,
        "distance",
        "synthetic-authorized-source",
        "observation://distance",
    )
    resolution = engine.resolve_authority(
        entity_type="activity",
        metric_reference="distance",
        candidates=(candidate,),
    )
...
def test_checkpoint_failure_preserves_evidence_and_retry_replays_safely() -> None:
    ...
    with pytest.raises(RuntimeError, match="checkpoint unavailable"):
        SynchronizationEngine(
            evidence_sink=store,
            checkpoint_store=Checkpoints(fail=True),
        ).run(
            adapter=adapter,
            connection_id="athlete-connection",
            mode=SyncMode.INCREMENTAL,
